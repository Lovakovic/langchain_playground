...
const passwordValidatorTool = tool(
  async ({ password, username }) => {
    console.log(
      `🔐 Validating password with ${password.length} characters${username ? ` for user: ${username}` : ''}`,
    );
    ...
    if (password.length < 12) {
      errors.push(
        `Password is too short (${password.length} characters). Minimum length is 12 characters.`,
      );
    }

    if (password.length > 128) {
      errors.push(
        `Password is too long (${password.length} characters). Maximum length is 128 characters.`,
      );
    }
    ...
    if (commonPatterns.some((pattern) => pattern.test(password))) {
      errors.push(
        'Password contains common weak patterns. Avoid repeated characters, sequential patterns, or common words.',
      );
    }
    ...
    if (username && password.toLowerCase().includes(username.toLowerCase())) {
      errors.push(
        `Password cannot contain the username "${username}". Please choose a different password.`,
      );
    }
    ...
    if (foundWords.length > 0) {
      errors.push(
        `Password contains common dictionary words: ${foundWords.join(', ')}. Avoid using common words.`,
      );
    }
  ...
  },
  {
    ...
    description:
      'Validate passwords against strict security requirements. Always try to validate any password provided, even if it seems obviously weak - the tool will provide detailed feedback on what needs to be improved.',
...
export async function createErrorHandlingAgent(
  checkpointer?: BaseCheckpointSaver,
): Promise<ErrorHandlingAgentGraph> {
...
async function main() {
  ...
  console.log(
    "🤖 Assistant: Hello! I'm a password security assistant. I can validate passwords against strict security requirements and help you understand what makes a password strong. What password would you like me to check?",
  );
