...
const WindowManagedState = Annotation.Root({
  ...
  stats: Annotation<{ totalProcessed: number; avgSize: number }>({
    reducer: (current, update) => ({
      ...
      avgSize:
        (current.avgSize * current.totalProcessed + update.avgSize * update.totalProcessed) /
...
async function windowManagedPipeline() {
  ...
  let state: typeof WindowManagedState.State = {
    recentData: [],
    stats: { totalProcessed: 0, avgSize: 0 },
    messages: [],
  };
  for (let i = 0; i < 5; i++) {
    console.log(`\n📦 Iteration ${i + 1}`);
...
async function streamingPipeline() {
  ...
  // Process data stream
  for await (const { index } of streamLargeData(50, 10)) {
