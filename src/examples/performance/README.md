...
Comprehensive benchmarking script measuring React agent compilation speed and
serialized size. Tests with/without MemorySaver checkpointer.

**Execution modes:**

- **Sequential** (default) - N iterations, statistical analysis
  (mean/min/max/stddev)
- **Parallel** - Stress test with N concurrent compilations, throughput
  calculation

**Metrics:**

- Compilation time (high-resolution via `perf_hooks`)
- Serialized size (V8 serialize + recursive deep-size with circular ref
  handling)
