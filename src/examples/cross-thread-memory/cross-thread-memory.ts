...
const saveUserInfoSchema = z.object({
  infoType: z
    .enum(['name', 'occupation', 'note', 'preference'])
    .describe('Type of information to save'),
  value: z.string().describe('The value to save'),
  preferenceKey: z
    .string()
    .optional()
    .describe("Key for preference (only used when infoType is 'preference')"),
});
...
const saveUserInfoTool = tool(
  ...
  },
  {
    name: 'save_user_info',
    description:
      'Save information about the user to persistent memory (name, occupation, notes, or preferences)',
...
async function callModelWithMemory(
  ...
  const messagesWithSystem = [{ role: 'system', content: systemPrompt }, ...state.messages];
...
async function main() {
  ...
  console.log(
    "🤖 Assistant: Hello! I'm your personal assistant with persistent memory. I can remember information about you across all our conversations. Feel free to tell me about yourself!",
  );
  ...
  rl.on('line', async (line) => {
    ...
    if (userInput.toLowerCase() === '/new_session') {
      ...
      console.log(
        "🤖 Assistant: I'm ready for our new conversation! I still remember everything about you from before.",
      );
      rl.setPrompt(`\n💬 [${sessions[currentSessionIndex]?.name}] You: `);
      rl.prompt();
      return;
    ...
    if (userInput.toLowerCase() === '/list_sessions') {
      ...
      sessions.forEach((session, index) => {
        ...
        console.log(
          `  ${index + 1}. ${session.name} - Started ${session.created.toLocaleString()}${current}`,
        );
      });
      rl.prompt();
      return;
