...
async function fetchRedditData(state: typeof SharedAnalysisState.State) {
  ...
  try {
    ...
    return {
      ...
      messages: [
        new AIMessage(
          `Reddit: Found ${posts.length} posts with average score ${avgScore.toFixed(0)}`,
        ),
      ],
    };
  } catch (error) {
...
async function analyzeRedditQuality(state: typeof SharedAnalysisState.State) {
  ...
  const quality =
    state.sentimentScore > 0.5 ? 'high' : state.sentimentScore > 0.2 ? 'medium' : 'low';
...
async function fetchTwitterData(state: typeof SharedAnalysisState.State) {
  ...
  return {
    ...
    messages: [
      new AIMessage(
        `Twitter: Analyzed ${mockTweets.length} tweets with ${avgEngagement.toFixed(0)} avg engagement`,
      ),
    ],
  };
}
...
async function analyzeTwitterQuality(state: typeof SharedAnalysisState.State) {
  ...
  const quality =
    state.sentimentScore > 0.7 ? 'high' : state.sentimentScore > 0.3 ? 'medium' : 'low';
...
async function fetchNewsData(state: typeof SharedAnalysisState.State) {
  ...
  try {
    ...
    return {
      ...
      messages: [
        new AIMessage(
          `News: Found ${articles.length} articles with ${avgPoints.toFixed(0)} avg points`,
        ),
      ],
    };
  } catch (error) {
...
async function analyzeNewsQuality(state: typeof SharedAnalysisState.State) {
  ...
  const quality =
    state.sentimentScore > 0.6 ? 'high' : state.sentimentScore > 0.3 ? 'medium' : 'low';
