  ...
  return {
    ...
    executionTimeline: [
      createTimelineEntry(
        'quick_validate',
        'QUICK',
        `Completed validation (${issues.length} issues)`,
      ),
    ],
  };
}
...
async function qaStep5(_state: typeof PipelineState.State) {
  ...
  const qaScore =
    (Object.values(qaChecks).filter((v) => v).length / Object.keys(qaChecks).length) * 100;
  ...
  return {
    ...
    executionTimeline: [createTimelineEntry('qa_step5', 'QA', `QA complete (Score: ${qaScore}%)`)],
  };
}
...
async function externalStep3(state: typeof PipelineState.State) {
  ...
  return {
    executionTimeline: [createTimelineEntry('external_step3', 'EXTERNAL', 'Secondary API success')],
  };
}
...
async function externalStep4(state: typeof PipelineState.State) {
  ...
  return {
    ...
    executionTimeline: [createTimelineEntry('external_step4', 'EXTERNAL', 'Integration complete')],
  };
}
