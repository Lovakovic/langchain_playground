...
async function initialSearchNode(state: typeof ResearchState.State) {
  ...
  try {
    ...
    console.log(
      `✅ Found ${searchResult.results.length} results (${searchTime}s, $${cost.toFixed(2)})`,
    );
...
async function main() {
  ...
  while (true) {
    ...
    if (choice === '1') {
      ...
      await graph.invoke({ topic }, { configurable: { thread_id: threadId } });
      ...
      const firstTask = currentState.tasks[0];
      if (
        currentState.tasks.length > 0 &&
        firstTask?.interrupts &&
        firstTask.interrupts.length > 0
      ) {
      ...
      if (index >= 0 && index < uniqueThreads.length) {
        ...
        if (currentState.values.totalCost > 0) {
          ...
          console.log(
            `💰 Previous searches preserved (saved $${currentState.values.totalCost.toFixed(2)})`,
          );
        }
        ...
        if (currentState.values.status === 'awaiting_feedback') {
          ...
          const newFirstTask = newState.tasks[0];
          if (
            newState.tasks.length > 0 &&
            newFirstTask?.interrupts &&
            newFirstTask.interrupts.length > 0
          ) {
      ...
      if (allCheckpoints.length === 0) {
        ...
        for (const [threadId, checkpoints] of threadGroups) {
          ...
          console.log(
            `   Latest Status: ${latestCheckpoint.checkpoint?.channel_values?.status || 'Unknown'}`,
          );
          console.log(`   Total Checkpoints: ${checkpoints.length}`);
          console.log(
            `   Cost: $${(latestCheckpoint.checkpoint?.channel_values?.totalCost || 0).toFixed(2)}`,
          );
