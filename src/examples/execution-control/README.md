...
Eight patterns for aborting/timing out graph execution on a simulated data
pipeline:
...
5. **User-cancellable + timeout** - `AbortSignal.any()` combines manual +
   automatic
...
**Key:** Requires `@langchain/core >= 0.2.20`, works with both `invoke()` and
`stream()`
