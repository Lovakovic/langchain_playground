...
function printRunTree(node: RunNode, indent: string = '', isLast: boolean = true): void {
  ...
  node.events.forEach((event, i) => {
    ...
    const dataPreview =
      typeof event.data === 'object' ? Object.keys(event.data).join(', ') : String(event.data);
...
function printTimeline(events: CustomEvent[]): void {
  ...
  sortedEvents.forEach((event) => {
    const eventTime = new Date(event.timestamp);
    const timeParts = eventTime.toISOString().split('T');
    const timeStr = timeParts[1] ? timeParts[1].slice(0, 12) : eventTime.toISOString();
...
if (require.main === module) {
  ...
  if (fileArgs.length > 0) {
    ...
    if (!providedFile) {
      console.error('Error: Invalid file argument');
      process.exit(1);
    }
