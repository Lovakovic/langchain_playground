...
async function analyzeRedditQuality(state: typeof SubgraphInternalState.State) {
  ...
  const quality = state.sentiment > 0.5 ? 'high' : state.sentiment > 0.2 ? 'medium' : 'low';
...
async function fetchTwitterData(state: typeof SubgraphInternalState.State) {
  ...
  const avgEngagement =
    mockTweets.reduce((acc, tweet) => acc + tweet.likes + tweet.retweets, 0) /
    (mockTweets.length || 1);
...
async function analyzeTwitterQuality(state: typeof SubgraphInternalState.State) {
  ...
  const quality = state.sentiment > 0.7 ? 'high' : state.sentiment > 0.3 ? 'medium' : 'low';
...
async function analyzeNewsQuality(state: typeof SubgraphInternalState.State) {
  ...
  const quality = state.sentiment > 0.6 ? 'high' : state.sentiment > 0.3 ? 'medium' : 'low';
...
async function aggregateResults(state: typeof ContentAnalysisState.State) {
  ...
  const overallQuality =
    qualities.filter((q) => q === 'high').length >= 2
      ? 'high'
      : qualities.filter((q) => q === 'medium').length >= 2
        ? 'medium'
        : 'low';
...
async function runFixedDemo() {
  ...
  try {
    ...
    console.log('\n📊 Results Summary:');
    console.log(
      `- Reddit: Sentiment ${result.redditSentiment.toFixed(2)}, Quality: ${result.redditQuality}`,
    );
    console.log(
      `- Twitter: Sentiment ${result.twitterSentiment.toFixed(2)}, Quality: ${result.twitterQuality}`,
    );
    console.log(
      `- News: Sentiment ${result.newsSentiment.toFixed(2)}, Quality: ${result.newsQuality}`,
    );
