...
export class HierarchicalToolTracer extends BaseTracer {
  ...
  override onChainStart(run: Run): void {
    ...
    this.allRuns.set(run.id, {
      ...
      graphLayer:
        this.graphStack.length > 0 ? this.graphStack[this.graphStack.length - 1] : undefined,
    });
  ...
  private buildHierarchyDisplay(_toolRunId: string): {
    ...
    const graphLayers: [string, string, string] = [
...
function createResearchSubgraph() {
  ...
  async function topicAnalyzerNode(state: typeof ResearchState.State) {
    ...
    console.log(
      `[Topic Analyzer] Analyzing topic: ${topic}. Depth: ${isComplex ? 'deep' : 'shallow'}`,
    );
    ...
    return {
      ...
      messages: [
        new AIMessage(`Analyzing topic: ${topic}. Depth: ${isComplex ? 'deep' : 'shallow'}`),
      ],
    };
  }
  ...
  async function quickSummaryNode(state: typeof ResearchState.State) {
    return {
      messages: [
        new AIMessage(`Quick summary for ${state.topic}: This is a straightforward topic.`),
      ],
...
function createMainGraph() {
  ...
  async function finalReportNode(state: typeof ResearchState.State) {
    ...
    return {
      messages: [
        new AIMessage(
          `Research complete! Analyzed ${findingsCount} sources. Topic: ${state.topic || 'unknown'}`,
        ),
      ],
    };
  }
...
async function main() {
  ...
  // Test queries
  const queries = ['quantum computing applications', 'web development basics'];
