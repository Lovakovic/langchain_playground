...
export class FileCallbackHandler extends BaseTracer {
  ...
  override onChainStart(run: Run) {
    ...
    this.log(`[chain/start] [${crumbs}] Entering Chain run with input:`, run.inputs);
  }
  ...
  override onLLMStart(run: Run) {
    ...
    const inputs =
      'prompts' in run.inputs
        ...
        : run.inputs;
    this.log(`[llm/start] [${crumbs}] Entering LLM run with input:`, inputs);
  }
  ...
  override onToolStart(run: Run) {
    const crumbs = this.getBreadcrumbs(run);
    this.log(`[tool/start] [${crumbs}] Entering Tool run with input:`, run.inputs['input']);
  }
  ...
  override onRetrieverStart(run: Run) {
    const crumbs = this.getBreadcrumbs(run);
    this.log(`[retriever/start] [${crumbs}] Entering Retriever run with input:`, run.inputs);
  }
