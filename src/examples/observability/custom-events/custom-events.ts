...
class HierarchyAwareEventHandler extends BaseTracer {
  ...
  private runHierarchy: Map<string, { parentRunId?: string; name: string; type: string }> =
    new Map();
...
async function createGraphExample() {
  const analyzeNode = async (state: typeof MessagesAnnotation.State) => {
    ...
    const sentimentAnalyzer = RunnableLambda.from(async (text: string) => {
      ...
      const sentiment =
        text.includes('happy') || text.includes('good')
          ? 'positive'
          : text.includes('sad') || text.includes('bad')
            ? 'negative'
            : 'neutral';
...
async function main() {
  ...
  const logFile = path.join(
    logsDir,
    `custom-events-${new Date().toISOString().replace(/:/g, '-')}.jsonl`,
  );
  ...
  try {
    await errorExample.invoke(
      { value: 25, operation: 'sqrt' },
      {
        callbacks: [eventHandler],
      },
    );
...
export async function mainWithWebImport() {
  ...
  const webExample = RunnableLambda.from(async (value: string, config?: RunnableConfig) => {
