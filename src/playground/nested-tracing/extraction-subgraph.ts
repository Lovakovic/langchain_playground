 ...
 */
async function fileProcessingNode(
  state: typeof ExtractionState.State,
): Promise<Partial<typeof ExtractionState.State>> {
  try {
 ...
 */
async function structureAnalysisNode(
  state: typeof ExtractionState.State,
): Promise<Partial<typeof ExtractionState.State>> {
  ...
  try {
    ...
    const model = createVertexAIModel({ model: 'gemini-2.5-flash', temperature: 0.0 }).bindTools(
      [structureTool],
      { tool_choice: 'any' },
    );
    ...
    const messages = [
      new SystemMessage(
        'Analyze the menu content and identify its structural sections. Use the tool to save your analysis.',
      ),
 ...
 */
async function itemExtractionNode(
  state: typeof ExtractionState.State,
): Promise<Partial<typeof ExtractionState.State>> {
  ...
  try {
    ...
    const sectionPromises = menuStructure.map(async (section, index) => {
      try {
        ...
        const messages = [
          new SystemMessage(
            `Extract menu items from the "${section.name}" section. Use the tool to save extracted items.`,
          ),
 ...
 */
async function formatOutputNode(
  state: typeof ExtractionState.State,
): Promise<Partial<typeof ExtractionState.State>> {
