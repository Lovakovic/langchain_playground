...
const nodeToPhaseMap: Record<string, { phase: ProcessingPhase; message: string }> = {
  ...
  initial_setup: {
    phase: ProcessingPhase.FILE_PROCESSING,
  ...
  category_enrichment: {
    phase: ProcessingPhase.ENRICHMENT_1,
    message: 'Categorizing menu items...',
  },
  merge_phase1_enrichments: {
    phase: ProcessingPhase.ENRICHMENT_1,
    message: 'Merging Phase 1 enrichments...',
  },
  prepare_phase2_enrichment: {
    phase: ProcessingPhase.ENRICHMENT_2,
    message: 'Preparing Phase 2 enrichment...',
  },
...
export class NestedTracer extends BaseTracer {
  name = 'nested_tracer';
  override ignoreCustomEvent = false; // We want to capture custom events
  ...
  override onLLMEnd(run: Run): void {
    ...
    if (tokenUsage) {
      ...
      const totalTokens =
        tokenUsage.total_tokens || tokenUsage.totalTokens || inputTokens + outputTokens;
  ...
  public getCustomEventsByName(eventName: string): CapturedEvent[] {
    return this.getCustomEvents().filter((event) => event.metadata?.['eventName'] === eventName);
  }
  ...
  public getExecutionSummary(): string {
    ...
    const customEventsByLevel = customEvents.reduce(
      (acc, event) => {
...
${Object.entries(customEventsByName)
  .map(([name, count]) => `  ${name}: ${count} events`)
  .join('\n')}

Custom Events by Graph Level:
${Object.entries(customEventsByLevel)
  .map(([level, count]) => `  Level ${level}: ${count} events`)
  .join('\n')}
