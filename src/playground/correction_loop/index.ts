...
class ErrorManager {
  ...
  getSummary(): string {
    ...
    return `❌ ${this.errors.size} errors: ${Object.entries(byCategory)
...
const updateConfigTool = tool(
  async ({ updates }, config?: RunnableConfig<RuntimeDeps>) => {
    ...
    if (
      updates.toLowerCase().includes('password') ||
      updates.toLowerCase().includes('compliance') ||
      updates.toLowerCase().includes('logging')
    ) {
  ...
  },
  {
    name: 'update_config',
    description:
      "Update configuration to fix validation errors. Mention which categories you're fixing (security/performance/compliance).",
...
// Model node
async function callModel(state: typeof ConfigState.State, config: RunnableConfig<RuntimeDeps>) {
  ...
  const tools = [
    ...
    performanceValidateTool,
    complianceValidateTool,
    finalSummaryTool,
  ];
...
// Routing function
function shouldContinue(state: typeof ConfigState.State, config: RunnableConfig<RuntimeDeps>) {
...
async function main() {
  ...
  console.log(
    "🤖 Tell me what kind of web server configuration you need, and I'll create and validate it!",
  );
  rl.prompt();
